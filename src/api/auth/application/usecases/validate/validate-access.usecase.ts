...
export class ValidateAccessUseCase {
	...
	public async execute(dto: ValidateItemAccessDTO): Promise<void> {
		...
		const hasModulePermission = this.hasModulePermission(
			user,
			dto.module,
			dto.permission,
		);
		const hasOwnership =
			dto.ownerUuid ?
				this.hasOwnership(user, new Set([dto.ownerUuid]))
			:	true;
		const hasTeamOwnership =
			dto.ownerTeamUuid ?
				this.hasTeamOwnership(user, new Set([dto.ownerTeamUuid]))
			:	true;
	...
	public async executeBulk(dto: ValidateItemAccessBulkDTO): Promise<void> {
		...
		const hasOwnership =
			dto.ownerUuids ?
				this.hasOwnership(user, new Set(dto.ownerUuids))
			:	true;
		const hasTeamOwnership =
			dto.ownerTeamUuids ?
				this.hasTeamOwnership(user, new Set(dto.ownerTeamUuids))
			:	true;
		...
		logger.info(
			"Use case ValidateAccessUseCase in bulk completed successfully",
		);
	}
	...
	private hasOwnership(
		user: UserWithRolesPermissionsAndTeams,
		ownerUuid: Set<string>,
	) {
		const userUuid = user.uuid;
		return ownerUuid.has(userUuid);
	}

	private hasTeamOwnership(
		user: UserWithRolesPermissionsAndTeams,
		ownerTeamUuids: Set<string>,
	) {
		const userTeamUuids = user.teams;
		return ownerTeamUuids
			.values()
			.some((teamUuid) => userTeamUuids.has(teamUuid));
	}
}
