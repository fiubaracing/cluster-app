...
import { FindUserByUuidWithRolesAndPermissionsUseCase } from "@/api/users/application/usecases/find-user-by-uuid-with-roles-and-permissions.usecase";

interface CreateContextUseCaseDependencies {
    jwtRepository?: JWTRepository;
    findUserByUuidWithRolesAndPermissions?: FindUserByUuidWithRolesAndPermissionsUseCase;
}

export class CreateContextUseCase {
    private readonly jwtRepository: JWTRepository;
    private readonly findUserByUuidWithRolesAndPermissions: FindUserByUuidWithRolesAndPermissionsUseCase;

    constructor(deps?: CreateContextUseCaseDependencies) {
        this.jwtRepository = deps?.jwtRepository ?? new JWTRepositoryImpl();
        this.findUserByUuidWithRolesAndPermissions = deps?.findUserByUuidWithRolesAndPermissions ?? new FindUserByUuidWithRolesAndPermissionsUseCase();
    }
    ...
    public async execute(token: string): Promise<Context> {
        const payload = await this.jwtRepository.decodeToken<AccessTokenPayload>(token);
        const user = await this.findUserByUuidWithRolesAndPermissions.execute(payload.uuid);
