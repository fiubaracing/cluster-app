...
import { FindShallowUserByUuidUseCase } from "@/api/users/application/usecases/find/find-shallow-user-by-uuid.usecase";
import { logger } from "@/api/shared/infrastructure/config/logger";
import { GenerateTokensUseCase } from "../generate/generate-tokens.usecase";
import { Auth } from "../../../domain/models/auth";
import {
	deleteCached,
	getCached,
} from "@/api/shared/infrastructure/config/redis";
...
export class RefreshTokenUseCase {
	...
	constructor(deps?: RefreshTokenUseCaseDependencies) {
		...
		this.findShallowUserByUuidUseCase =
			deps?.findShallowUserByUuidUseCase ??
			new FindShallowUserByUuidUseCase();
	...
	public async execute(refreshToken: string): Promise<Auth> {
		...
		const { uuid, sessionId }: RefreshTokenPayload =
			await this.jwtRepository.decodeToken(refreshToken);
